...
\section{Conclusioni}
  In conclusione questa esperienza ha parzialmente confermato la validità del modello teorico ideale del filtro
  crossover.
  La stima della frequenza di cross mediante lo sfasamento è risultata compatibile con quella prevista così come il
  comportamento dello sfasamento relativo tra Tweeter e Woofer.
  L'analisi delle ampiezze ha prodotto risultati in disaccordo con la teoria, portando ad una stima della frequenza di
  cross non compatibile con quella attesa.

  Anche sulla base di osservazioni qualitative sulla forma del grafico del $V_{in}$ riteniamo che siano intervenuti
  effetti reali non previsti dal nostro modello, per la precisione delle capacità, dovute alla millefori della ELVIS, in
  parallelo con le resistenze di carico (le più significative).
  La loro considerazione nel modello avrebbe però richiesto molti calcoli e la riacquisizione dei dati.

  Costruendo il circuito su una base che non introduce questi effetti potrebbe però ridurre le discrepanze tra modello
  teorico e comportamento osservato.
